window.addEventListener("load", function(){
    ...
    for(let i = 0; i < navbar.children.length; i++){
        ...
        document.styleSheets[0].insertRule(`@keyframes move-link-${i}{0%{transform: translate(${getTranslateX(navbar.children[i])}px,${getTranslateY(navbar.children[i])}px);}${(80/navbar.children.length) * i}%{transform: translate(${getTranslateX(navbar.children[i])}px ,0px);}80%,100%{transform: translateX(0);}}`)
